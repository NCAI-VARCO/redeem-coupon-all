...
- `http://localhost:3000/admin` 접속 후 ADMIN_TOKEN 값 입력
- 또는 `http://localhost:3000/admin?token=ADMIN_TOKEN값`
...
3. Settings → Environment Variables에 아래 추가
   ...
   - `FIRESTORE_DATABASE_ID`  # 기본 DB면 생략 가능
   - `ADMIN_TOKEN`
...
`redeem-coupon-all` 프로젝트로 바꾸려면 Vercel의 `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY`를 새 Firebase 프로젝트의 서비스 계정 키 값으로 모두 교체해야 합니다. 기존 `redeem-america` 서비스 계정 키를 그대로 쓰면 새 프로젝트에 연결되지 않습니다.
...
`관리자 토큰이 올바르지 않습니다.` 메시지는 Firebase 프로젝트 연결 오류가 아니라, Vercel 환경변수 `ADMIN_TOKEN`과 화면에서 입력한 토큰이 다를 때 발생합니다.

확인 순서:
1. Vercel → Project → Settings → Environment Variables에서 `ADMIN_TOKEN`이 있는지 확인
2. 값 앞뒤에 공백이나 따옴표가 들어가지 않았는지 확인
3. Production / Preview / Development 중 실제 배포 환경에 들어갔는지 확인
4. 환경변수 수정 후 반드시 Redeploy
5. `/admin`으로 접속해 같은 값을 입력
